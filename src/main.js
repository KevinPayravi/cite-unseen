...
let convByVar = null;
let vueApp = null;
let currentSuggestionCitation = null;
 ...
 */
function findReliabilityMatch(coins, filteredCategorizedRules) {
...
function createSettingsButton(minerva) {
    ...
    if (minerva) {
        ...
        label.textContent = convByVar(CiteUnseenI18n.settingsButton);
    ...
    settingsButton.setAttribute('title', convByVar(CiteUnseenI18n.settingsButtonTooltip));
...
function createDashboardForReflist(reflistData) {
    ...
    clearAllButton.innerText = convByVar(CiteUnseenI18n.clearAllFilters);
    clearAllButton.setAttribute('title', convByVar(CiteUnseenI18n.clearAllFiltersTooltip));
...
function updateDashboardCategories(dashboard, categoryCounts) {
    ...
    for (const category of categoryTypes) {
        ...
        if (count > 0) {
            ...
            countIcon.alt = convByVar(CiteUnseenI18n.categoryHints[category]);
            ...
            const categoryLabel = convByVar(CiteUnseenI18n.categoryLabels[category]);
            ...
            countNode.setAttribute('title', convByVar(CiteUnseenI18n.filterToggleTooltip));
...
function updateFilteredCountForReflist(dashboard, visibleCount, totalCount) {
    ...
    const citationText = totalCount === 1 ?
        convByVar(CiteUnseenI18n.citationSingular) :
        convByVar(CiteUnseenI18n.citationPlural);

    if (visibleCount === totalCount) {
        totalElement.innerText = baseText + convByVar(CiteUnseenI18n.totalCitations) + ' ' + totalCount + ' ' + citationText;
    } else {
        const filterInfo = dashboard.reflistData.selectedCategories.size > 1 ?
            " (" + dashboard.reflistData.selectedCategories.size + ' ' + convByVar(CiteUnseenI18n.filtersActive) + ")" :
            "";

        totalElement.innerText = baseText +
            convByVar(CiteUnseenI18n.showing) + ' ' + visibleCount + ' ' +
            convByVar(CiteUnseenI18n.of) + ' ' + totalCount + ' ' +
...
function injectStyles() {
    ...
    style.type = 'text/css';
    style.textContent = citeUnseenStyles;
...
function createSettingsDialog() {
    mw.loader.using('@wikimedia/codex').then(function (require) {
        ...
        const app = Vue.createMwApp({
            i18n: {
                documentationLink: convByVar(CiteUnseenI18n.documentationLink),
                viewSettingsFrom: convByVar(CiteUnseenI18n.viewSettingsFrom),
                loading: convByVar(CiteUnseenI18n.loading),
                metaWikiGlobal: convByVar(CiteUnseenI18n.metaWikiGlobal),
                local: convByVar(CiteUnseenI18n.local),
                localSettingGuidance: convByVar(CiteUnseenI18n.localSettingGuidance),
                tabGeneral: convByVar(CiteUnseenI18n.tabGeneral),
                tabCategories: convByVar(CiteUnseenI18n.tabCategories),
                tabIgnoreDomains: convByVar(CiteUnseenI18n.tabIgnoreDomains),
                tabAdditionalDomains: convByVar(CiteUnseenI18n.tabAdditionalDomains),
                tabAdditionalStrings: convByVar(CiteUnseenI18n.tabAdditionalStrings),
                categoriesTabGuidance: convByVar(CiteUnseenI18n.categoriesTabGuidance),
                ignoreDomainsTabGuidance: convByVar(CiteUnseenI18n.ignoreDomainsTabGuidance),
                additionalDomainsTabGuidance: convByVar(CiteUnseenI18n.additionalDomainsTabGuidance),
                additionalStringsTabGuidance: convByVar(CiteUnseenI18n.additionalStringsTabGuidance),
                enableDisableCategories: convByVar(CiteUnseenI18n.enableDisableCategories),
                showDashboard: convByVar(CiteUnseenI18n.showDashboard),
                showSuggestionsButton: convByVar(CiteUnseenI18n.showSuggestionsButton),
                hideSocialMediaReliabilityRatings: convByVar(CiteUnseenI18n.hideSocialMediaReliabilityRatings),
                showOtherLanguageReliabilityRatings: convByVar(CiteUnseenI18n.showOtherLanguageReliabilityRatings),
                domainsToIgnore: convByVar(CiteUnseenI18n.domainsToIgnore),
                additionalDomains: convByVar(CiteUnseenI18n.additionalDomains),
                additionalUrlStrings: convByVar(CiteUnseenI18n.additionalUrlStrings)
            },
            data() {
                return {
                    open: true,
            ...
            computed: {
                ...
                dialogTitle() {
                    return convByVar(CiteUnseenI18n.settingsDialogTitle);
                },
                primaryAction() {
                    return {
                        label: this.isSaving ? convByVar(CiteUnseenI18n.saving) : convByVar(CiteUnseenI18n.save),
                ...
                defaultAction() {
                    return {
                        label: convByVar(CiteUnseenI18n.cancel)
                    };
                },
            ...
            methods: {
                    ...
                    loadCustomRulesFromWiki(wikiHost, {})
                        .then(rules => {
                            if (this.target === 'meta') {
                                ...
                                _metaRules = rules ? { ...defaultRules, ...rules } : defaultRules;
                            } else {
                                ...
                                _localRules = rules ? { ...emptyRules, ...rules } : emptyRules;
                            }
                ...
                saveSettings() {
                    ...
                    if (allCorrections.length > 0) {
                        ...
                        const correctionMessage = convByVar(CiteUnseenI18n.domainsCorrectedMessage) + '\n\n' + correctionMessages.join('\n');
                    ...
                    if (validationErrors.length > 0) {
                        this.isSaving = false;
                        const errorMessage = convByVar(CiteUnseenI18n.invalidDomainFormatMessage) + '\n\n' + validationErrors.join('\n');
                    ...
                    saveSettingsToWiki(
                        ...
                        (result) => {
                            this.closeDialog();
                            if (confirm(convByVar(CiteUnseenI18n.settingsSavedSuccess))) {
                                location.reload();
                            }
                        },
                        (error) => {
                            mw.notify(convByVar(CiteUnseenI18n.settingsSaveError) + ' ' + (error.message || error), {
                                type: 'error',
                                title: '[Cite Unseen]'
                            });
                        }
        ...
        vueApp = app.mount('#cite-unseen-dialog-mount');
    }).catch(function (error) {
        console.error('[Cite Unseen] Failed to load Codex dependencies:', error);
        mw.notify(convByVar(CiteUnseenI18n.dialogLoadError), { type: 'error', title: '[Cite Unseen]' });
    });
}
...
function createSuggestionsToggleButton(minerva, reflistData) {
    ...
    if (minerva) {
        ...
        // Suggestion label
        const label = document.createElement('span');
        label.textContent = convByVar(CiteUnseenI18n.suggestionsToggleButton);
    ...
    suggestionsToggleButton.setAttribute('title', convByVar(CiteUnseenI18n.suggestionsToggleTooltip));
    ...
    reflistData.refs.forEach(ref => {
        ...
        if (suggestionsMode) {
            if (!plusSign) {
                ...
                plusSign.setAttribute('title', convByVar(CiteUnseenI18n.suggestCategorization));
...
function openSuggestionDialog(citationRef) {
    ...
    currentSuggestionCitation = citationRef;
...
function createSuggestionDialog(citationRef) {
    ...
    mw.loader.using('@wikimedia/codex').then(function (require) {
        ...
        const app = Vue.createMwApp({
            i18n: {
                dialogTitle: convByVar(CiteUnseenI18n.suggestionDialogTitle),
                guidance: convByVar(CiteUnseenI18n.suggestionsDialogGuidance),
                sourceUrl: convByVar(CiteUnseenI18n.sourceUrl),
                suggestedCategories: convByVar(CiteUnseenI18n.suggestedCategories),
                selectAtLeastOne: convByVar(CiteUnseenI18n.selectAtLeastOneCategory),
                optionalComment: convByVar(CiteUnseenI18n.optionalComment),
                commentPlaceholder: convByVar(CiteUnseenI18n.commentPlaceholder),
                reliabilityGuidance: convByVar(CiteUnseenI18n.suggestionsDialogReliabilityGuidance),
                reliabilityProjects: convByVar(CiteUnseenI18n.reliabilityProjects),
                submit: convByVar(CiteUnseenI18n.submit),
                submitting: convByVar(CiteUnseenI18n.submitting),
                cancel: convByVar(CiteUnseenI18n.cancel)
            },
            data() {
                return {
                    open: true,
            ...
            computed: {
                ...
                reliabilityProjects() {
                    const pageLinks = Object.values(citeUnseenData.citeUnseenSourceToPageMapping);
                    const projects = [];
            ...
            methods: {
                ...
                submitSuggestion() {
                    ...
                    submitSuggestionToMeta(this.sourceUrl, this.selectedCategories, this.comment)
                        .then(() => {
                            this.closeDialog();
                            mw.notify(convByVar(CiteUnseenI18n.suggestionSubmitted), {
                                type: 'success',
                                title: '[Cite Unseen]'
                            });
                        })
                        .catch((error) => {
                            ...
                            mw.notify(convByVar(CiteUnseenI18n.suggestionSubmitError) + ' ' + (error.message || error), {
                                type: 'error',
                                title: '[Cite Unseen]'
                            });
                        })
 ...
 */
function submitSuggestionToMeta(sourceUrl, selectedCategories, comment) {
    return new Promise((resolve, reject) => {
        try {
            ...
            const citationRef = currentSuggestionCitation;
